...
export function calculatePercentageChange(
   ...
   const currentWeekMembers = members.filter(
     (member) => member.joined > previousWeekEnd
   );
 
   const currentWeekPayments = vehicles.filter(
     (vehicle) => vehicle.timestamp > previousWeekEnd
   );
