      ...
      <>
         ...
         >
            <Flex width={'100%'}>
               ...
               <Text fontSize={'sm'} marginLeft={'auto'}>{`${new Date(user.joined * 1000).toDateString()}`}</Text>
            </Flex>
...
const WaitList = ({ useMobStyle, users }: { users: MemberDocType[], useMobStyle?: boolean } = { users: [], useMobStyle: false }) => {
   ...
   useEffect(()=> {
         setWaitListedUsers(users.filter(u => {
            return u.permitted == null && !u.deleted;
         }));
         setWaitListedUsersCount(users.filter(u => {
            return u.permitted == null && !u.deleted;
         }).length);
   }, [users]);
