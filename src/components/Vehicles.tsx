    ...
    <>
      <Flex width="100%" flexDir="column" gap="1rem" height="100%">
        <ContentHeader
          ...
          heading="Vehicles (438)"
        />
        <Flex gap={"0.1rem"}>
          <VehcilesList vehciles={vehciles} />
...
const VehcilesList = ({ vehciles }: { vehciles: Array<any> }) => {
  return (
    <>
      <Flex
        flex={3}
      ...
      >
        <Flex
          ...
          width={"100%"}
        >
          {vehciles.map((vehicle, idx) => {
            return <VehicleData imgUrl={vehicle.cutout} key={idx} />;
          })}
        </Flex>
        <Flex flexBasis={"17%"}>
...
const VehicleData = ({ imgUrl }: { imgUrl: string }) => {
  return (
    <>
      {/*eslint-disable-next-line @next/next/no-img-element */}
      <Box rounded={"lg"} p={"1rem"} bg={"var(--grey-color)"}>
        <Image
          src={imgUrl}
          alt="vehicle_image"
          height={212}
        ...
        <Flex p={"0.1rem"} flexDir={"column"}>
          <Heading fontSize={"md"}>Honda Civic</Heading>
